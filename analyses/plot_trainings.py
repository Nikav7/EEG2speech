...
def add_vlines():
    for x, c in zip(vlines_x, vlines_colors):
        plt.axvline(x=x, color=c, linestyle='--', alpha=0.5)

def add_min_val_vlines(val_series, epochs_series, n=10, color='blue', min_epoch=59):
    mask = epochs_series > min_epoch
    best_idx = val_series[mask].nsmallest(n).index
    print(f"Best {n} epochs (after epoch {min_epoch}): {epochs_series.loc[best_idx].values}")
...
val_cer_gt,val_cer_recon = df['val_cer_gt'],df['val_cer_recon']
...
plt.xlabel('Epochs')
plt.ylabel('Adversarial loss (G, given D)')
plt.title('Train and Validation Adversarial loss, (G, given D) (Generator 22kHz)')
add_vlines()
plt.legend()
...
add_vlines()
#add_min_val_vlines(val_lossG, epochs)
plt.legend()
...
plt.xlabel('Epochs')
plt.ylabel('Loss Discriminator (D, given G)')
...
plt.plot(epochs, train_cer_recon, label='CER gen train', color='red')
plt.plot(epochs, val_cer_gt, label='CER gt val', color='orange')
plt.plot(epochs, val_cer_recon, label='CER gen val', color='red')
plt.xlabel('Epochs')
